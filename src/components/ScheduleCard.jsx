...
export default function ScheduleCard({
  ...
  const isClassBooked =
    Array.isArray(bookedClasses) &&
    bookedClasses.some((c) => {
      const cid = c?.fields?.["Class ID"];
      const classIdMatch = Array.isArray(cid)
        ? String(cid[0]) === String(classInfo.id)
        : false;
  ...
  return (
    <div
      key={classInfo.id}
      className="bg-black shadow rounded-lg p-2 mb-2 flex flex-col gap-1 w-full max-w-[170px] mx-auto sm:max-w-[180px]"
    >
      ...
      <CheckoutButton
        ...
        popular={false}
        onBooked={(classInfo) =>
          setBookedClasses([...bookedClasses, classInfo])
        }
      />
    </div>
  );
